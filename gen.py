...
def generate_canny_image(image):
    ...
    image_np = np.array(image.convert("L"))  # 转为灰度图
    canny_edges = cv2.Canny(image_np, 100, 200)
    canny_edges = np.repeat(canny_edges[:, :, None], 3, axis=2)  # 转为三通道
...
def generate_segmentation_sam(image_path, sam_checkpoint, model_type="vit_h"):
    ...
    for idx, mask in enumerate(masks):
        combined_mask[mask['segmentation']] = idx + 1 
...
def main():
    ...
    segmentation_sam.save("segmentation_sam.png")


    print("Images saved: Canny, Depth, SAM Segmentation")
